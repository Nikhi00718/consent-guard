...
from consentguard.config import TrainingConfig, project_path
from consentguard.perception.dataset import VisualRedactionsDataset, detection_collate, target_for_model
from consentguard.perception.metrics import evaluate_instance_segmentation
from consentguard.runtime import (
    atomic_json_dump,
    atomic_link_or_copy,
    atomic_torch_save,
...
def build_datasets(config: TrainingConfig) -> tuple[VisualRedactionsDataset, VisualRedactionsDataset | None]:
...
def build_data_loaders(config: TrainingConfig) -> tuple[DataLoader, DataLoader | None]:
    ...
    if data["sampling"] == "class_balanced":
        ...
        power = float(data["class_balance_power"])
        raw_weights = []
        for record in train_dataset.records:
        ...
        maximum = median * float(data["max_class_balance_ratio"])
        weights = torch.tensor([min(weight, maximum) for weight in raw_weights], dtype=torch.double)
...
def build_scheduler(optimizer: torch.optim.Optimizer, config: dict[str, Any]):
    ...
    if name == "cosine":
        return torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=int(config.get("t_max", 20)), eta_min=float(config.get("eta_min", 0.0))
        )
    if name == "none":
    ...
    def _train_epoch(self, epoch: int) -> dict[str, float]:
        ...
        for batch_index, (images, targets) in enumerate(self.train_loader):
            ...
            try:
                ...
                raise RuntimeError(
                    "CUDA ran out of memory. Use configs/train_maskrcnn_4gb.yaml, close GPU applications, "
