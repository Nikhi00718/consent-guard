...
CCPD_DATASET = "consentguard-ccpd2020"
CODE_DATASET = "consentguard-training-code-ccpd2020"
...
def stage_assets(username: str) -> tuple[Path, Path, Path]:
    source = ROOT / "data/raw/ccpd/CCPD2020"
    code_archive = ROOT / "artifacts/kaggle/consentguard-training-code-ccpd2020.zip"
    kernel_source = ROOT / "notebooks/kaggle"
    if not source.is_dir():
        raise FileNotFoundError(f"CCPD2020 source is missing: {source}")
    if not code_archive.is_file():
        raise FileNotFoundError(
            "Build consentguard-training-code-ccpd2020.zip with prepare_kaggle_bundle.py first"
        )

    data_dir = ROOT / "artifacts/kaggle" / CCPD_DATASET
    shutil.copytree(source, data_dir / "CCPD2020", dirs_exist_ok=True)
    _write_metadata(
        data_dir,
        dataset_id=f"{username}/{CCPD_DATASET}",
        title="ConsentGuard official CCPD2020 plate data",
        description=(
            "Official CCPD2020 train, validation, and test folders. The training kernel "
            "uses only train and val; the test folder is retained but never converted or used."
        ),
    )
    ...
    metadata.update(
        {
            ...
            "dataset_sources": [f"{username}/{CCPD_DATASET}", f"{username}/{CODE_DATASET}"],
        }
    )
    (kernel_dir / "kernel-metadata.json").write_text(
        json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    return data_dir, code_dir, kernel_dir


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--username", default=USERNAME)
    parser.add_argument("--upload", action="store_true", help="Create the private CCPD and code datasets")
    ...
    data_dir, code_dir, kernel_dir = stage_assets(args.username)
    if args.upload:
        _kaggle("datasets", "create", "-p", str(data_dir), "--dir-mode", "zip")
        ...
        json.dumps(
            {
                ...
                "ccpd_dataset": f"{args.username}/{CCPD_DATASET}",
