...
LEAKAGE_QUARANTINE = (
    ROOT / "main_project" / "configs" / "stage_01_data" / "cross_split_leakage_quarantine.json"
)
...
def main() -> None:
    ...
    negative_records_by_split: Counter[str] = Counter()
    ...
    for row in manifest_rows:
        ...
        # A true profile-negative image is useful supervision for false-positive
        # control.  It is safe to retain only when the source annotation did not
        # contain a selected attribute.  If selected annotations existed but all
        # failed validation, treating the image as background would create false
        # negative labels, so that record remains omitted.
        if not instances and selected_attribute_count > 0:
            omitted.append(
                {
                    "image_id": image_id,
                    "split": split,
                    "reason": "selected_profile_instances_invalid",
        ...
        if negative_for_profile:
            negative_records_by_split[split] += 1
        ...
        records_by_split[split].append(
            {
                ...
                "negative_for_profile": negative_for_profile,
                "instances": instances,
                "excluded_instances": [],
            }
        )
    ...
    summary = {
        ...
        "negative_records_by_split": {
            split: negative_records_by_split[split]
            for split in ("train2017", "val2017", "test2017")
        },
