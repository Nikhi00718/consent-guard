...
def main() -> None:
    ...
    for path in sorted(data.glob("records_*.jsonl")):
        ...
        for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            if width < 1 or height < 1 or not isinstance(instances, list):
                add_invalid(invalid, image_id, "invalid_dimensions_or_instances")
                continue
            if not instances and record.get("negative_for_profile") is not True:
                add_invalid(invalid, image_id, "empty_instances_without_negative_marker")
                continue
