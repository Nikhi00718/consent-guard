...
from consentguard.shared.runtime import atomic_json_dump
...
def predict_union_mask(
    ...
    *,
    ...
    min_area_pixels_by_class: dict[int, int] | None = None,
    dilation_pixels_by_class: dict[int, int] | None = None,
    ...
    for mapping_name, mapping in (
        ...
        ("min_area_pixels_by_class", min_area_pixels_by_class),
        ("dilation_pixels_by_class", dilation_pixels_by_class),
    ):
        if mapping is not None and any(int(class_id) < 1 for class_id in mapping):
            raise ValueError(f"{mapping_name} class IDs must be positive")
    ...
    if score_thresholds:
        thresholds = torch.tensor(
            [score_thresholds.get(int(label), score_threshold) for label in prediction["labels"]],
            dtype=prediction["scores"].dtype,
            device=prediction["scores"].device,
        )
        keep = prediction["scores"] >= thresholds
    else:
    ...
    for index in indices.tolist():
        ...
        minimum_area = (min_area_pixels_by_class or {}).get(class_id, 1)
        if int(mask.sum()) < minimum_area:
            continue
        class_dilation = (dilation_pixels_by_class or {}).get(class_id, dilation_pixels)
        if class_dilation > 0:
            size = 2 * class_dilation + 1
        ...
        detections.append(
            {
                ...
                "score_threshold_used": (score_thresholds or {}).get(class_id, score_threshold),
                "dilation_pixels_used": class_dilation,
