...
| 03 — Specialists | Fine-tuned, experimental | YuNet face, LPD-YuNet plate, PP-OCRv3 ONNX text geometry, zxing-cpp, and one-class learned specialists are wired into the evidence pipeline. The Roboflow Indian plate v1 publisher split failed leakage audit; a grouped, no-test full-scene candidate (1,904 train / 1,824 validation) is prepared for Kaggle. Face validation is promising; plate and handwriting remain experimental. |
...
Earlier face, plate, handwriting, baseline, and CCPD/India Kaggle experiments
are complete. The next isolated job is the grouped full-scene plate adaptation
described in `reports/PROJECT_AUDIT_AND_EXECUTION_PLAN_2026-08-30.md`. Kaggle
account credentials remain outside the repository.
